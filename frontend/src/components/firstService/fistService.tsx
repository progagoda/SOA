...
export const FirstService = () => {
    ...
    const handleCreateSpaceMarine = async (): Promise<any> => {
        ...
        setOpen(false);
        const {isError} = await queryClient.fetchQuery(['createSpaceMarines', {spaceMarine}], createSpaceMarine).catch(
            // messageApi.open({
            //     type: 'error',
            //     content: 'ji'
            // })
    ...
    // @ts-ignore
    useEffect(() => {
        queryClient.invalidateQueries('getSpaceMarines').catch(
            (reason) => {
                messageApi.open({
                    type: 'error',
                    content: reason
    ...
    const onChange = async (pagination: any, filters: any, sorter: any, extra: any) => {
        ...
        await getSpaceMarines(sorter, filters, pagination);
  ...
  const footer = (
    ...
    return (
        <>
            ...
            } loading={isLoading} bordered dataSource={data} columns={columnsConfig} onChange={onChange}/>
        </>
    );
};
